...
P2-B1 candidate manifests are stored in [`/manifests`](../manifests). P2-B2-A adds one tightly scoped, research-only admission in the external approval index at [`/registry/approved-manifests.json`](../registry/approved-manifests.json); it is not a general approval for DiTFake or its upstream images. See the [P2-B1 admission report](p2b1-data-admission-report.md) and the [P2-B2-A licence record](p2b2a-licence-record.md).
...
| DiTFake mini (P2-B2-A) | [DiTFake revision `ca9ea06`](https://huggingface.co/datasets/Jouesmak/DiTFake/tree/ca9ea06c8f926c3a11ca4b657074cc7cbb99e5c7) | 24 fixed files: 12 COCO-referenced real, 12 FLUX generated | Original RGB images | DiTFake test/FLUX.1-schnell branch; fixed repository revision | Dataset card declares Apache-2.0; inherited COCO/source rights require further review before expansion | Yes, only for the recorded frozen-feature research baseline | No | approved for this exact research-only manifest |
...
`backend/models/datasets.py` exposes `require_training_approval`. It rejects every manifest unless `approval_status` is exactly `approved`, `training_permitted` is true, and its versioned SHA-256 is present in the trusted approval allow-list. The P2-A repository-owned synthetic **feature** fixture tests plumbing only. The P2-B2-A DiTFake mini manifest is the sole approved downloaded-image input, and only for its recorded frozen-feature research baseline.
