...
3. Populate the `ai-photo-reconstructor-runtime` Secret through the approved secret manager/CSI driver. It may contain `APP_API_KEYS`, or be omitted when OIDC is fully configured. Do not commit a Secret manifest or use `kubectl create secret` in shell history.
