...
- **Validation Program**: a controlled, versioned protocol for assessing whether the institution can use the existing system within a stated scope. It is not a production launch, a judicial certification, or a claim of universal detection performance.
- **Validation batch**: one frozen execution of the Validation Program, bound to a scenario matrix version, admitted sample hashes, system/Registry versions, review records, and a final validation report.
- **Validation sample**: one licence-reviewed, hash-bound image record used in a Validation batch, with a documented scenario, curated label, source, and known limitations. Its label is test ground truth for the declared protocol, not a runtime conclusion.
- **Review decision**: the accountable human disposition of an Authentication report: `ACCEPT_WITHIN_SCOPE`, `RETURN_FOR_MORE_EVIDENCE`, or `EXCLUDE_OUT_OF_SCOPE`. It records reviewer, reason, and timestamp; it is not a judicial finding.
- **Review agreement**: the documented level of agreement between independent reviewers applying the Validation Program's review rubric. It measures procedure consistency, not the truth of image origin.
- **Shadow Pilot**: a private, parallel and non-consequential evaluation in which the Authentication system analyzes a permitted copy while the institution's normal human process continues unchanged. The system output cannot alter, delay, approve, reject, or otherwise affect that process.
- **Pilot dataset record**: a consent/licence-reviewed, hash-bound record of a Shadow Pilot sample with a case reference, source, scenario, and analysis version. It is not a production case record and may not contain samples of unknown origin.
- **Reviewer feedback**: the auditable comparison between a system assessment and the normal human-process disposition for a Shadow Pilot case. `agreement` may be `AGREE`, `DISAGREE`, or `UNDETERMINED`; it is a process-observation value, not ground truth.
- **Pilot failure record**: an immutable record of an observed false alert, missed alert, insufficient evidence, input error, or out-of-scope outcome. It is retained for governance and never removed to improve a reported metric.
- **Model Validation Program**: a controlled benchmark and review protocol that evaluates one detector candidate within a declared population. It does not itself approve a model, make it a formal Provider, or authorize an institutional deployment.
- **Validated detector candidate**: a detector candidate that has completed its declared Model Validation Program and independent review with retained failures and stated scope. It is distinct from Registry `APPROVED` status and remains unavailable to formal reports until the separate Registry and Provider-admission requirements are met.
- **Benchmark manifest**: a frozen, repeatably accessible list of licence-reviewed validation samples, hashes, scenarios, labels, transformation conditions, and split assignments for one validation run.
- **Validation failure record**: an append-only record for a benchmark false positive, false negative, out-of-scope result, or uncertain result, linked to its sample hash and run version. It is never deleted to improve reported performance.
- **Validation dataset registry**: the controlled, hash-gated registry of candidate benchmark samples and their source, licence, scenario, label, collection metadata, and split. An un-hashed sample is not a validation sample and cannot enter a run.
- **Signed validation package**: a canonical, signed review package that binds a verified Provider Admission chain to a frozen dataset manifest, configuration, report hashes, metrics schema, and limitations. It is an independent-review artifact, not a model approval.
- **Readiness gate**: a pre-scoring condition that must be satisfied before a P6-B run can produce candidate-validation metrics. Failure of a gate yields a retained blocked/rejected record, not an inferred result.
- **Red-team transformation**: a deliberately adverse but permitted input change, such as metadata removal, JPEG recompression, screenshotting, cropping, resizing, or editing, used to test whether the system preserves uncertainty or declared out-of-scope behavior.
